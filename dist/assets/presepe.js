/* Prezzo in euro. Collega l'URL Hotmart quando il prodotto IT è pronto. */
const EUR_PRICE = 12;
const HOTMART_CHECKOUT_URL = "https://pay.hotmart.com/I107595155E?checkoutMode=10";
...
if (!region) region = TIMEZONE_COUNTRY[timezone] || "IT";

let currency = COUNTRY_CURRENCY[region] || "EUR";

const priceFormatter = (selectedCurrency) => new Intl.NumberFormat(locale.startsWith("it") ? "it-IT" : locale, {
  style: "currency",
...
const paintPrice = (rates = FALLBACK_RATES) => {
  ...
  if (note) {
    note.textContent = currency === "EUR"
      ? "Prezzo in euro. Pagamento unico."
      : `Circa ${formatted}. Hotmart conferma l'importo finale prima del pagamento.`;
  }
};
  ...
  button.addEventListener("click", () => {
    ...
    if (typeof window.fbq === "function") {
      window.fbq("track", "InitiateCheckout", {
        value: EUR_PRICE,
        currency: "EUR",
        content_name: "Presepe in Pannolenci"
      });
    }
    if (url) {
...
document.querySelectorAll('a[href="#offerta"]').forEach((link) =>
  link.addEventListener("click", (event) => {
    event.preventDefault();
    document.querySelector("#offerta")?.scrollIntoView({ behavior: "smooth" });
  })
);

const sticky = document.querySelector("#sticky-cta");
const pieces = document.querySelector("#pezzi");
const offer = document.querySelector("#offerta");
